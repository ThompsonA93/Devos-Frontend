...
const IDBContextProvider = (props) => {
    ...
    /**
     * TODO -- Update a single entry after voting
     * @param {*} id of the record
     * @param {*} choice as yes/no, influencing what is updated
     * @returns 
     */
    function updateContractTableVotes(id, choice) {
        return new Promise(
            function (resolve, reject) {
                var idbOpen = indexedDB.open("DevosDB", 1);
                idbOpen.onsuccess = function (event) {
                    ...
                    var tx = db.transaction("ballotInfo", "readwrite");
                    var store = tx.objectStore("ballotInfo");
                    var entryReadRequest = store.get(id);
                    entryReadRequest.onsuccess = function (event) {
                        console.log("IDB::Attempt to update entry", event.target.result);
                        // TODO 
                        /*
                        address: "0xfC204598c0c10a07a589923b1228Dd083db94DDc"
                        creator: "0x4898426fBfc21F6c1a855631DaE0891B2c9e5e8A"
                        endTime: "7/20/2022, 2:22:24 PM"
                        id: 0
                        metaInfo: "Lorem Ipsum"
                        proVotes: "0"
                        startTime: "6/20/2022, 2:22:24 PM"
    ...
    return (
        <IDBContext.Provider value={{ initiateIDBStorages, writeToAddressArchiveTable, readFromAddressArchiveTable, writeToContractTable, readFromContractTable, updateContractTableVotes }}>
            {props.children}
        </IDBContext.Provider>
    )
}
